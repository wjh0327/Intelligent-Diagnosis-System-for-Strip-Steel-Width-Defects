...
# ==================== 四元组处理（优化版） ====================
def build_quadruple_texts(quad: Dict) -> Dict:
    device = quad.get("设备", "")
    fault_type = quad.get("故障类型", "")
    feature_str = quad.get("故障特征", "").replace("，", ",")
    solution = quad.get("诊断方案", "")

    description = f"设备：{device}。故障类型：{fault_type}。故障特征向量：{feature_str}。诊断方案：{solution}"
    keywords = f"设备:{device} 故障类型:{fault_type} 特征:{feature_str}"

    vector = None
    try:
        vec = [float(x.strip()) for x in feature_str.split(",")]
        if len(vec) == 12:
            vector = vec
        else:
            st.warning(f"四元组故障特征维度不为12，已跳过: {feature_str}")
    except:
        pass

    # 标准化特征向量（与 build_kb.py 一致）
    scaler = get_scaler()
    if vector is not None and scaler is not None:
        try:
            vector = scaler.transform([vector])[0].tolist()
        except Exception:
            pass  # 标准化失败则保留原始值
...
def insert_quadruples(quads: List[Dict]):
    ...
    for q in quads:
        processed = build_quadruple_texts(q)
        desc_texts.append(processed["description"])
        kw_texts.append(processed["keywords"])
        if processed["feature_vector"] is not None:
            feat_vectors.append(processed["feature_vector"])
