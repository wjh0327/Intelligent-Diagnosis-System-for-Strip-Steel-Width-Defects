...
def create_tools(model_info, retriever, feature_names, predict_single, retrieve_context,
    ...
    def retrieve_knowledge_tool(query: str) -> str:
        ...
        try:
            ...
            if not context or not context.strip():
                return f"[检索提示] 知识库未返回与「{query}」直接匹配的内容。可尝试用故障中文名称、机理关键词或调整方法等角度重新检索。"
            return context
        except Exception as e:
    ...
    def search_similar_cases_tool(features: str) -> str:
        """
        根据12维特征向量搜索历史相似故障案例。
        输入为逗号分隔的12个原始特征值，必须按模型特征顺序：
        FMWIDTHACTHOT, FMWTARGETCOL, FMWTARGETHOT, PDIWIDTHTOL,
        RDWTARGETTOTAL, RMWIDTHACTHOT, is_DM, is_FM, is_HT, is_NA, is_RM, is_WS
        （与 classify_fault_tool 的输入顺序一致）
        """
        try:
            feat_list = [float(x.strip()) for x in features.split(",")]
            if len(feat_list) != 12:
    ...
    # ------------------------------------------------------------------
    @tool
    def query_hist_diag_tool(features: str) -> str:
        """根据12维特征值查询该特征向量的历史诊断记录（跨会话追溯）。
        输入为逗号分隔的12个原始特征值，与 classify_fault_tool 输入一致。"""
        try:
            feat_list = [float(x.strip()) for x in features.replace("，", ",").split(",")]
            if len(feat_list) != 12:
                return f"错误：需要恰好12个特征值，当前输入了 {len(feat_list)} 个。"
            rec = MEMORY.get_diag_by_features(feat_list)
            if not rec:
                return "未找到该特征向量的历史诊断记录。"
            return json.dumps({
