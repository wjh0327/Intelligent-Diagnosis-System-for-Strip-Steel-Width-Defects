...
def create_tools(model_info, retriever, feature_names, predict_single, retrieve_context,
    ...
    def batch_csv_diagnosis_tool(csv_path: str) -> str:
        ...
        try:
            ...
            summary = f"共处理 {len(diag_list)} 条数据，故障分布："
            for fault, cnt in fault_counts.most_common():
                summary += f"\n  {fault}: {cnt} 条"

            # 追加前3条详细结果
            details = ["\n前3条详细结果："]
            for i, d in enumerate(diag_list[:3]):
                details.append(
                    f"第{i+1}条: 故障类型={d['fault_cn']}, 描述={d['fault_desc']}"
                )

            return summary + "\n".join(details)
        except Exception as e:
            return f"批量CSV诊断失败: {str(e)}"
