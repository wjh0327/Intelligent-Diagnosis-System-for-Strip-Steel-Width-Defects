...
def build_agent_graph(
    ...
    base_prompt = (
        ...
        "不要重复调用 batch_csv_diagnosis_tool 或 diagnose_strip_tool。\n"
        "可用工具：\n"
        "1. diagnose_strip_tool：输入单条带钢记录JSON（STRIPNO+6工艺参数+3段曲线，字段见工具说明）预测故障类型（仅允许调用一次！）\n"
        "2. retrieve_knowledge_tool：根据文本查询搜索知识库\n"
        "3. search_similar_cases_tool：输入单条带钢记录JSON查找相似历史案例\n"
        "4. get_feature_importance_tool：查询某故障类型的关键特征\n"
        "5. batch_csv_diagnosis_tool：对上传的CSV文件进行批量诊断\n"
        "6. query_hist_diag_tool：输入单条带钢记录JSON查询其历史诊断记录\n\n"
        "决策规则：\n"
        "- **CSV批量诊断优先**：如果用户提供了CSV文件路径（csv_path非空），且历史中尚未执行过批量诊断（batch_done为False），"
        "  你必须首先调用 batch_csv_diagnosis_tool，禁止在此时调用 diagnose_strip_tool 逐条处理。\n"
        ...
        "  但严禁再次调用 diagnose_strip_tool。\n"
        "- 如果用户提供了单条带钢记录（JSON或粘贴的数据行，含曲线数据，非CSV场景），且历史中未对该卷进行诊断，则调用 diagnose_strip_tool。\n"
