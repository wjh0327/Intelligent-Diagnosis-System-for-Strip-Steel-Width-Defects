...
MILVUS_DB_PATH = "./milvus_kb.db"               # 数据库文件
...
def build_quadruple_texts(quad: Dict, scaler) -> Dict[str, Any]:
    """
    将单条四元组转为文本和标准化特征向量
    """
    device = quad.get("设备", "")
    ...
    raw_vector = []
    try:
        raw_vector = [float(x.strip()) for x in feature_str.split(",") if x.strip()]
    except ValueError:
...
def main():
    ...
    for chunk in doc_chunks:
        if len(chunk.page_content) <= MAX_CHUNK_CHARS:
            ...
            for sub in sub_texts:
                safe_chunks.append(Document(page_content=sub, metadata=chunk.metadata))
    print(f"分块安全检查后，块数从 {len(doc_chunks)} → {len(safe_chunks)}")
    doc_chunks = safe_chunks

    # 5. 加载并处理四元组（标准化特征向量）
    print("\n===== 2/4 处理四元组 =====")
    quadruples_raw = load_quadruples(QUADRUPLE_JSON)
    quad_processed = [build_quadruple_texts(q, scaler) for q in quadruples_raw]
    ...
    index_know.add_index("embedding", index_type="IVF_FLAT",
